...
def main():
    ...
    for pop_name in population_epsilons:
        ...
        save_particles(kept_particles, sim_media_names=['M3', 'M2'], output_dir=pop_out_dir)
...
def main_2():
    ...
    for pop_name in population_epsilons:
        ...
        for idx, row in particles_df.iterrows():
            ...
            init_populations = np.load(f"{dir}/particle_init_populations.npy")[p_idx]
            k_values = np.load(f"{dir}/particle_k_values.npy")[p_idx]
            max_exchange = np.load(f"{dir}/particle_max_exchange.npy")[p_idx]
            toxin = np.load(f"{dir}/particle_toxin.npy")[p_idx]
