...
class Community:
    def __init__(
        ...
        logger.info(
            f"Setting toxin mat, mem usage (mb): {utils.get_mem_usage()}"
        )
        self.set_toxin_mat(
        ...
        self.set_biomass_rate_constraints(
            np.ones(len(self.populations)) * 1000
        )
    ...
    def sample_parameters_from_prior(self):
        ...
        if self.biomass_rate_constraint_prior is not None:
            if isinstance(self.biomass_rate_constraint_prior, SampleCombinationParticles):
                biomass_constraints = self.biomass_rate_constraint_prior.sample(
                    self.model_names, data_field="biomass_rate_constraints"
                )

            else:
                biomass_constraints = self.biomass_rate_constraint_prior.sample(size=[n_populations,])
    ...
    def generate_parameter_vector(self):
        ...
        param_vec = np.concatenate(
            [initial_concs_vec, k_val_vec, max_exchange_vec, biomass_constraints_vec, toxin_mat], axis=1
        ).reshape(-1, 1)
    ...
    def load_parameter_vector(self, parameter_vec):
        ...
        self.set_toxin_mat(
            parameter_vec[
                n_variables
                + n_k_vals
                + n_max_exchange_vals + n_biomass_constraint_vals : n_variables
                + n_k_vals
                + n_max_exchange_vals
                + n_biomass_constraint_vals
                + n_toxin_vals
    ...
    def sim_step(self, y):
        ...
        for idx, pop in enumerate(self.populations):
            ...
            pop.model.reactions.get_by_id(self.objective_reaction_keys[idx]).bounds = (0.0, self.biomass_constraints[idx])
